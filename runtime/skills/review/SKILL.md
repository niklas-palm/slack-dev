...
- **Establish the baseline.** Run the repo's own check command *before* you critique anything, so a
  ...
  `npm run check`, `npm test`, `make test`, `pytest`. **Run the repo's install step first** \u2014 nothing is
  pre-installed for a repo you cloned, and a missing install fails in ways that look like defects in the
  change you're reviewing.
